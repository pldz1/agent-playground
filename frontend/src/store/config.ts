import { AppSettings, ModelConfigIssue, ModelConfigStatus, ModelRole } from '../types';
...
export const modelConfig = (settings: AppSettings): ModelConfigStatus => {
  ...
  const findModel = (label: string | null | undefined) =>
    label ? (settings.models.find((model) => model.label === label) ?? null) : null;
  ...
  const configured = REQUIRED_ROLES.every((role) => !issues.some((issue) => issue.role === role));
